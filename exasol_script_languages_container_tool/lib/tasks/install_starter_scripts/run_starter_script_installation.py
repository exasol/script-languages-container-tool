...
def run_starter_script_installation(
    install_path: Path, target_script_path: Path, force_install: bool
):
    print(f"Installing to {install_path}")
    if target_script_path.exists() and not force_install:
        print(
            f"The installation directory for exaslct at {install_path} already exists."
        )
        ...
        if answer == "yes":
            ...
            print(
                "Can't continue with the installation, because the installation directory already exists."
            )
            sys.exit(1)
    elif force_install:
        shutil.rmtree(target_script_path)
    ...
    exaslct_script_path = (
        ...
        / "starter_scripts/exaslct_install_template.sh"
    )
    ...
    os.chmod(
        target_script_path / "exaslct.sh", 0o0764
    )  # Full access (rwx) for owner, for group only r/w access, readonly fo rall others

    exaslct_symlink_path = install_path / "exaslct"

    exaslct_symlin_exists = (
        exaslct_symlink_path.is_symlink()
        or exaslct_symlink_path.exists()
        or exaslct_symlink_path.is_file()
    )

    if exaslct_symlin_exists and not force_install:
        print(
            f"The path for the symlink to exaslct at {exaslct_symlink_path} already exists."
        )
        print("Do you want to remove it and continue with installation?")
        answer = input("yes/no:")
        ...
        if answer == "yes":
            ...
            print(
                "Can't continue with the installation, because the path to exaslct symlink already exists."
            )
            print(
                "You can change the path to exaslct symlink by setting the environment variable EXASLCT_SYM_LINK_PATH"
            )
            sys.exit(1)
    elif force_install:
        exaslct_symlink_path.unlink()
