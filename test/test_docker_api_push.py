...
class ApiDockerPushTest(unittest.TestCase):
    ...
    def test_docker_push(self):
        with LocalDockerRegistryContextManager(self.test_environment.name) as local_registry:
            ...
            image_infos = api.push(flavor_path=(str(self.test_environment.get_test_flavor()),),
            ...
            self.assertIn(str(self.test_environment.get_test_flavor()), image_infos)
            images = local_registry.images
            print("images", images)
            images_info_list = image_infos[str(self.test_environment.get_test_flavor())]
            images_info_list_tags = list(set(image_info.target_tag for image_info in images_info_list)).sort()
            self.assertEqual(images["tags"].sort(), images_info_list_tags,
