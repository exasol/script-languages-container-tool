...
class InstallStarterScriptTests(unittest.TestCase):
    ...
    def test_positive(self):
        with tempfile.TemporaryDirectory() as target_dir:
            ...
            exaslct_script_path = importlib_resources.files(MODULE_IDENTITY) / "starter_scripts"
    ...
    def _build_docker_runner(self):
        build_docker_runner_img_script = Path(__file__).parent.parent.absolute() / "scripts" / "build" / "build_docker_runner_image.sh"
        completed_process = subprocess.run([str(build_docker_runner_img_script)], stdout=subprocess.PIPE)
        completed_process.check_returncode()
        current_runner_image_name = completed_process.stdout.decode("utf-8").strip("\n")
        return current_runner_image_name

    def _build_docker_runner_release_tag(self, current_runner_image_name: str, script_dir: str):
        construct_docker_runner_image_script = Path(script_dir) / TARGET_EXASLCT_SCRIPTS_DIR / \
                                               "construct_docker_runner_image_name.sh"
        version = importlib_metadata.version(MODULE_IDENTITY)
        completed_process = subprocess.run(["bash", str(construct_docker_runner_image_script), f"{version}"],
                                           stdout=subprocess.PIPE)
        completed_process.check_returncode()
        target_docker_runner_image_name = completed_process.stdout.decode("utf-8").strip("\n")
        completed_process = subprocess.run(["docker", "tag",
                                            current_runner_image_name, target_docker_runner_image_name])
