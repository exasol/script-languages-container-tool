...
GEN_PKG_DIFF_CURRENT_DIRECTORY = GEN_PKG_DIFF_ROOT_DIRECTORY / "current"
GEN_PKG_DIFF_LAST_TAG_DIRECTORY = GEN_PKG_DIFF_ROOT_DIRECTORY / "last_tag"
GEN_PKG_DIFF_EXPECTED_RESULT_DIRECTORY = GEN_PKG_DIFF_ROOT_DIRECTORY / "expected_result"
GEN_PKG_DIFF_EXPECTED_RESULT_BUILD_STEP_DIRECTORY = GEN_PKG_DIFF_ROOT_DIRECTORY / "expected_result_build_step"
...
class ExaslctApiTestEnvironmentWithCleanup:
    ...
    def clean_all_images(self):
        api.clean_all_images(
            docker_repository_name=self._itde_api_test_environement.docker_repository_name
        )
...
class ExaslctTestEnvironmentWithCleanUp:
    ...
    def __init__(
        ...
        self._itde_cli_test_environment = (
            exaslct_test_environment.ExaslctTestEnvironment(
    ...
    def run_command(
        ...
        return self._itde_cli_test_environment.run_command(
            command=command,
    ...
    def clean_all_images(self):
        self.run_command(
            f"{self.executable} clean-all-images", use_flavor_path=False, clean=True
        )
